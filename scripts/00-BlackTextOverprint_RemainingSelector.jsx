/*
    ...
    K100とかがオーバープリントになってるかの確認！

    Illustrator 黒未オーバープリント残り候補 選択診断用
...
(function () {
    ...
    for (var i = 0; i < candidates.length; i++) {
        try {
            if (candidates[i].character) {
                // 文字単位候補は、最後に1文字選択すると他の選択が外れることがあるので、
                // 全体選択はTextFrameにする
                candidates[i].item.selected = true;
            } else {
                candidates[i].item.selected = true;
            }
        } catch (e1) {}
    ...
    function inspectPath(pathItem, parentItem) {
        ...
        try {
            if (pathItem.filled) {
                if (
                    isBlackColor(pathItem.fillColor) &&
            ...
            if (pathItem.stroked) {
                if (
                    isBlackColor(pathItem.strokeColor) &&
